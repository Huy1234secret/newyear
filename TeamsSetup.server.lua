		...
		onRoundPrepared = function(context)
			...
			local function buildRocketModel(originCF: CFrame)
				...
				if prefab and prefab:IsA("Model") then
					...
					rocketRoot = model:FindFirstChildWhichIsA("BasePart") or Instance.new("Part")
				else
					-- Fallback: simple cylinder + cone tip
				...
				rocketRoot.AssemblyLinearVelocity = Vector3.zero
				rocketRoot.AssemblyAngularVelocity = Vector3.zero
				rocketRoot.CanCollide = false
				rocketRoot.CanTouch = true
				rocketRoot:SetNetworkOwner(nil)
				return model, rocketRoot
			end

			local function createRocket(botState, targetHrp: BasePart)
				local origin = botState.ball and botState.ball.CFrame or CFrame.new(targetHrp.Position + Vector3.new(0, HOVER_HEIGHT, 0))
				...
				local function explode()
					...
					damageInRadius(pos, ROCKET_BLAST_RADIUS)
					if model.Parent then model:Destroy() end
				end

				local tConn
				tConn = root.Touched:Connect(function(hit)
			...
			local function spawnBot(index: number)
				...
				ball.Color = colors[killBotRandom:NextInteger(1, #colors)]
